...
<script lang="ts">
    ...
    import { store as lokasiStore } from '@/routes/admin/lokasi';
    ...
    let {
        ...
        jadwals,
        hariLiburs,
    }: { lokasis: L[]; jadwals: J[]; hariLiburs: H[] } = $props();
...
<div class="mx-auto grid max-w-4xl gap-4 p-5">
    <section class="g-tile">
        <h3>Lokasi absen</h3>
        {#each lokasis as l}<p>
                {l.nama} · {l.latitude}, {l.longitude} · {l.radius_meter} m
            </p>{/each}
        <form
            class="grid gap-2"
            onsubmit={(e) => {
        ...
        >
            <Input placeholder="Nama" bind:value={lokasi.nama} /><Input
                placeholder="Latitude"
                bind:value={lokasi.latitude}
            /><Input
                placeholder="Longitude"
                bind:value={lokasi.longitude}
            /><Input type="number" bind:value={lokasi.radius_meter} /><Button
                type="button"
                variant="outline"
                onclick={gps}>Pakai lokasi saya</Button
            ><Button type="submit" disabled={lokasi.processing}>Tambah</Button>
        </form>
    </section>
    <section class="g-tile">
        <h3>Jadwal kerja</h3>
        ...
        >
            {#each jadwal.jadwals as j, i}<div class="grid grid-cols-5 gap-2">
                    <span>{hari[j.day_of_week]}</span><Input
                        type="time"
                    ...
                    /><Input
                        type="number"
                        bind:value={jadwal.jadwals[i].toleransi_menit}
                    /><input
                        type="checkbox"
                        bind:checked={jadwal.jadwals[i].is_hari_kerja}
                    />
                </div>{/each}<Button type="submit" disabled={jadwal.processing}
    ...
    <section class="g-tile">
        <h3>Hari libur</h3>
        {#each hariLiburs as h}<p>
                {h.tanggal} · {h.nama}
                <Button
                    size="sm"
                    onclick={() => router.delete(liburDestroy(h.id).url)}
                    >Hapus</Button
                >
            </p>{/each}
        <form
        ...
        >
            <Input type="date" bind:value={libur.tanggal} /><Input
